...
public class MMsegIndicesAnalysis extends AbstractComponent {
	...
	@Inject
	public MMsegIndicesAnalysis(final Settings settings, IndicesAnalysisService indicesAnalysisService,
	        Environment env) {
		super(settings);
		final Dictionary dic = Dictionary.getInstance();

		indicesAnalysisService.analyzerProviderFactories().put(MMSEG,
		        new PreBuiltAnalyzerProviderFactory(MMSEG, AnalyzerScope.GLOBAL, new MMSegAnalyzer(dic)));

		indicesAnalysisService.analyzerProviderFactories().put(MMSEG_MAXWORD,
		        new PreBuiltAnalyzerProviderFactory(MMSEG_MAXWORD, AnalyzerScope.GLOBAL, new MaxWordAnalyzer(dic)));

		indicesAnalysisService.analyzerProviderFactories().put(MMSEG_COMPLEX,
		        new PreBuiltAnalyzerProviderFactory(MMSEG_COMPLEX, AnalyzerScope.GLOBAL, new ComplexAnalyzer(dic)));

		indicesAnalysisService.analyzerProviderFactories().put(MMSEG_SIMPLE,
		        new PreBuiltAnalyzerProviderFactory(MMSEG_SIMPLE, AnalyzerScope.GLOBAL, new SimpleAnalyzer(dic)));
